...
const ProductDetailPage: React.FC<ProductDetailPageProps> = ({
  ...
  const generateConfigurationCategories = useCallback((optionProducts: ConfigurableProductSchema[]) => {
    ...
    let displayOrder = 0;
    const newCategories = Array.from(categoryMap.entries()).map(([categoryName, options]) => ({
      ...
      options,
      maxSelections: categoryName.toLowerCase().includes('accessories') ? undefined : 1,
      minSelections: 0,
    ...
    console.log(`Created ${newCategories.length} configuration categories:`, newCategories.map(c => c.name));
  }, [product]);
  ...
  useEffect(() => {
    if (optionsLoaded && optionProducts.length > 0 && product) {
      ...
      console.log(`Updated product with ${optionProducts.length} option products`);
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      const optionsTotal = selectedOptionsList.reduce((sum: number, option: any) => {
        ...
        <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8">
          ...
          <nav className="flex mb-8" aria-label="Breadcrumb">
            <ol className="flex items-center space-x-4">
              ...
              <li>
                <span className="text-gray-900 font-medium">{product.title || product.name || 'Product'}</span>
              </li>
            </ol>
          </nav>
          ...
          <div className="mb-12">
            ...
            <div className="mt-8 flex justify-center">
              <button
                ...
                disabled={loading}
                className="bg-green-600 hover:bg-green-700 disabled:bg-gray-400 text-white px-8 py-3 rounded-lg font-medium transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 flex items-center justify-center"
              >
                {loading ? (
                  <>
            ...
            <div className="lg:col-span-2">
              {showSummary ? (
                <div className="bg-white rounded-lg shadow-md p-6">
                  ...
                  <div className="space-y-4">
                    <div className="border-b pb-4">
                      <h3 className="font-semibold text-gray-900">{product.title || product.name || 'Product'}</h3>
                      <p className="text-gray-600">Base Model</p>
                      <p className="text-lg font-bold text-blue-600">${parseFloat(product.price?.toString() || '0').toFixed(2)}</p>
                    </div>
                        ...
                        <div key={categoryId} className="border-b pb-4">
                          ...
                          {options.map(option => (
                            <div key={option.id} className="flex justify-between items-center py-2">
                              ...
                              <span className="font-medium">${parseFloat(option.price?.toString() || '0').toFixed(2)}</span>
                            </div>
                          ))}
                        </div>
                      );
                ...
                <div className="bg-white rounded-lg shadow-md p-8 text-center">
                  ...
                  <h3 className="text-xl font-semibold text-gray-900 mb-2">Configure Your {product.title || product.name || 'Product'}</h3>
...
const getIconForCategory = (categoryName: string): string => {
  ...
  if (name.includes('warranty') || name.includes('extended')) {
    ...
    return '🔧';
  } else if (name.includes('color') || name.includes('fabric') || name.includes('custom')) {
    return '🎨';
  } else if (name.includes('accessories') || name.includes('accessory')) {
    return '⚙️';
