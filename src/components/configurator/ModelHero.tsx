...
const ModelHero: React.FC<ModelHeroProps> = ({
  ...
  // Use proper null checking instead of truthy check for totalPrice
        ...
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            <div className="relative group">
              {/* aspect-square + object-contain + neutral bg = the whole chair
                  is always visible, never cropped, regardless of the source
              ...
              <div className="flex gap-2 mt-4 overflow-x-auto">
                {images.map((image, index) => (
                  <button
                    ...
                    className={`relative flex-shrink-0 w-16 h-16 rounded border-2 overflow-hidden bg-gray-50 ${
                      currentImageIndex === index ? 'border-brand-primary' : 'border-gray-200'
                    }`}
                  >
                    <Image
          ...
          <div className="flex-1">
            ...
            {model.shortDescription && (
              <div className="text-gray-600 mb-6 leading-relaxed">
                <RichContent content={model.shortDescription} className="prose prose-sm max-w-none text-gray-600" />
              </div>
            )}
            ...
            <div className="border-t border-gray-200 pt-6">
              <div className="flex items-start justify-between gap-4 mb-4">
                <div>
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-2xl font-bold transition-colors duration-500 ${
                        showRealTimePrice ? 'text-[#3fa2a3]' : 'text-gray-900'
                      }`}
                    >
                    ...
                    {showRealTimePrice && (
                      <span className="w-2 h-2 bg-brand-primary rounded-full animate-pulse" aria-hidden="true" />
                    )}

                    {hasOptionsPriceDelta && (
                  ...
                  {/* Real-time pricing indicator now sits on its own line above
                      the description, instead of running into it */}
                  {showRealTimePrice && (
                    <div className="flex items-center gap-1 text-xs text-brand-primary mt-1">
                      <span className="w-1.5 h-1.5 bg-brand-primary rounded-full inline-block" />
                  ...
                  {enhancedOptionsCount > 0 ? (
                    <p className="text-sm text-gray-600 mt-1">
                      ...
                      {showRealTimePrice &&
                        enhancedSelectedOptions.some((option) => option.selectedVariations.length > 0) && (
                          <span className="block text-orange-600 mt-0.5">
                            with{' '}
                            {enhancedSelectedOptions.reduce(
                              (sum, option) => sum + option.selectedVariations.length,
                              0
                            )}{' '}
                            variations
                          </span>
                    ...
                    <p className="text-sm text-gray-600 mt-1">Base product price</p>
                  )}
                </div>
                ...
                {(onPriceClick || onConfigurationClick) && (
                  <button
                    ...
                    className="shrink-0 text-brand-primary hover:opacity-80 text-sm font-medium whitespace-nowrap"
                  >
                ...
                <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mt-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-lg font-bold text-purple-800">${financingOption.monthlyPayment}/month</p>
              ...
              {showProgressIndicator && (
                <div className="bg-yellow-50 border border-orange-200 rounded-lg p-4 mt-4">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-sm font-medium text-orange-900">Configuration Progress</h4>
